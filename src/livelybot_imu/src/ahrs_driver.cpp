  ...
  while (ros::ok()&&!loop_flag)
  {
    ...
    if (head_type[0] == TYPE_AHRS)
    {
      ...
      if (device_type_ == 0)         //未经变换的原始数据
      {
        ...
        //cout << "q1: " << imu_data.orientation.w << ", q2: " << imu_data.orientation.x << ", q3: " << imu_data.orientation.y << ", q4: " << imu_data.orientation.z << endl;
      }
      ...
      double magyaw;
      magCalculateYaw(0, 0, magyaw, magx, magy, magz);
    ...
    else if (head_type[0] == TYPE_GEODETIC_POS)
    {
      ...
      // gps_data.longitude = 117;
      // gps_data.altitude = 100;
