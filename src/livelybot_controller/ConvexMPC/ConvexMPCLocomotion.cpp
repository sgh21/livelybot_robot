...
void ConvexMPCLocomotion::run(ControlFSMData &data)
{
  ...
  // pick gait
  Gait *gait = &standing;
  if (gaitNumber == 1) gait = &standing;
  else if (gaitNumber == 2) gait = &walking;
  ...
  for (int i = 0; i < 2; i++)//循环分别处理机器人的两只脚
  {
    ...
    if (firstSwing[i]) swingTimeRemaining[i] = swingTimes[i];
    else               swingTimeRemaining[i] -= dt;
    
    footSwingTrajectories[i].setHeight(0.1);//每只脚设置摆动轨迹的高度，这里设置为0.1米。
